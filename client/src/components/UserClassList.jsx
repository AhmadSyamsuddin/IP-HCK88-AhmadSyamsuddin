...
export default function UserClassList({ classItem, onDeleteSuccess }) {
  ...
  const { deletingClassId, error, deleteSuccess } = useSelector((state) => state.userClasses);
  const isDeleting = deletingClassId === classItem.id;
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        title="Leave Workout Class"
        message={`Are you sure you want to leave "${classItem.name}"? This action cannot be undone.`}
